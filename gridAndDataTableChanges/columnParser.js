...
function columnParser(i, $item) {
    ...
    column.template = parseTemplate($item, column);

    return column;
}

function parseTemplate($item, column) {
    const $tmp = $item.find(constants.COLUMN_TEMPLATE);
    if ($tmp.length > 0)
        return replaceTemplate($tmp.html());
    return  `<span>{{ ${column.ngModel} }}</span>`;
}
...
export function replaceTemplate(template) {
    return template
        ...
        .replaceAll(constants.COMPONENT_PREFIX, '')
}
